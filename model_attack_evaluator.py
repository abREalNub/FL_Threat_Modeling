...
def evaluate_model_attack(func):

    @functools.wraps(func)
    def _evaluate_infered_data_(
        server_model: FlexModel,
        test_data: Dataset,
        *args, 
        **kwargs
    ):
        def evaluate_infered(server_model, test_data,*args, **kwargs):
            try:              
                accuracy = func(server_model, test_data, *args, **kwargs)
            except ValueError:
                raise ValueError(
                        f"The decorated function: {func.__name__} must return one value: a accuracy."
                )
            return accuracy
        
        return evaluate_infered(server_model, test_data, *args, **kwargs) 

    return _evaluate_infered_data_


def inference_evaluator(evaluator_train_func, discriminator, generator, test_data, evaluation_func, data_to_evaluate):
    evaluator_train_func(discriminator, generator, test_data)
    accuracy = evaluation_func(discriminator, data_to_evaluate)
    return accuracy
...
def train_evaluator(discriminator, generator, test_data): #Convertir las inferencias en imágenes, o no
    n_epoch = 1
    n_clases = 10
    optimizerG = torch.optim.Adam(generator.parameters(), 0.0002, betas=(0.5, 0.999))
    optimizerD = torch.optim.SGD(filter(lambda p: p.requires_grad, discriminator.parameters()),lr= 0.1, momentum=0.9, weight_decay = 0.25)
    criterion_adv = nn.BCELoss()
    criterion_aux = nn.CrossEntropyLoss()

    test_dataloader = DataLoader(
        test_data, batch_size=256, shuffle=True, pin_memory=False
    )
    ...
    for n in range(n_epoch):
        for data, target in tqdm(test_dataloader):
            #froze_layer(discriminator.l_gan_logit)

            current_batchSize = data.size()[0]
            realLabel = torch.ones(current_batchSize)
            fakeLabel = torch.zeros(current_batchSize)

            predictR, predictRLabel = discriminator(data) #image from the real dataset
            loss_real_aux = criterion_aux(predictRLabel, target)
            loss_real_aux.backward()

            loss_real_adv = criterion_aux(predictR, realLabel)
            loss_real_adv.backward()

            optimizerD.zero_grad()
            optimizerG.zero_grad()
            optimizerD.step()
            real_score = predictR #Hasta acá el entrenamiento con imágenes reales

            #activate_layer(discriminator.l_gan_logit)

            latent_value = torch.normal(0,10,(current_batchSize, 128))

            gen_labels = torch.LongTensor(np.random.randint(0, n_clases, current_batchSize))

            fake_images = generator(latent_value , gen_labels) #generate a fake image
            predictF, predictFLabel = discriminator(fake_images)
            ...
            for i in range(6): 
                ...
                gen_labels = torch.LongTensor(np.random.randint(0, n_clases, current_batchSize))
                fake_images= generator(latent_value, gen_labels) #Generate a fake image
                predictG, predictLabel = discriminator(fake_images)
...
def discriminator_evalaution_function(discrminator, data_evaluate):

    criterion = nn.BCELoss()
    losses = []
    realLabel = torch.ones(data_evaluate.size()[0])
    with torch.no_grad():
        for data in data_evaluate:
            output = discrminator(data)
            losses.append(criterion(output, realLabel.long()).item())
            pred = output.data.max(1, keepdim=True)[1]
            test_acc += pred.eq(realLabel.data.view_as(pred)).long().cpu().sum().item()
    
    test_loss = sum(losses) / len(losses)
    test_acc /= data_evaluate.size()[0]
